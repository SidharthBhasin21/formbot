...
const Settings = ()=> {
    ...
    const handleUserUpdate = async () => {
        const data = await userUpdate(input, token);
        if (data) navigate(`/dashboard/${dashboardId}`);
    };
    ...
    const validateForm = (e) => {
        ...
        if (!isError) handleUserUpdate();
    };

    return (
        <main className={styles.settings}>
            <Link to={`/dashboard/${dashboardId}`}><img src="/icons/arrow-back.png" className="goback" alt="Go back" /></Link>
