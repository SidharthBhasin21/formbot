...
const Signup = () => {
    ...
    const userRegister = async () => {
        const data = await userSignupApi(input);
        console.log(data);
    ...
    const validateForm = (e) => {
        ...
        Object.keys(input).forEach((key) => {
            ...
            if (typeof element === "string" && element.trim().length === 0) {
                ...
                setError((error) => ({
                    ...error,
                    [key]: "Password must be 6 characters long",
                }));
    ...
    return (
        <main className={styles.auth}>
            ...
            <img
                className={`${styles.vector} ${styles.triangleLayer}`}
                src="/images/triangle.png"
                width={240}
                alt="triangle-layer"
            />
            <img
                className={`${styles.vector} ${styles.ellipsePink}`}
                src="/images/semi-pink.png"
                width={80}
                alt="ellipse-pink"
            />
            <img
                className={`${styles.vector} ${styles.ellipseYellow}`}
                src="/images/semi-yellow.png"
                height={80}
                alt="ellipse-yellow"
            />
        </main>
